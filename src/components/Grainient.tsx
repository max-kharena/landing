...
const Grainient = ({
  ...
  }, [color1, color2, color3, contrast, grainAmount]);
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      for (let i = 0; i < 3; i++) {
        current.color1[i] = lerp(current.color1[i], target.color1[i], transitionSpeed);
        current.color2[i] = lerp(current.color2[i], target.color2[i], transitionSpeed);
        current.color3[i] = lerp(current.color3[i], target.color3[i], transitionSpeed);
      }
      current.contrast = lerp(current.contrast, target.contrast, transitionSpeed);
      current.grainAmount = lerp(current.grainAmount, target.grainAmount, transitionSpeed);

      // Update uniforms
      const c1 = program.uniforms.uColor1.value as Float32Array;
      const c2 = program.uniforms.uColor2.value as Float32Array;
      const c3 = program.uniforms.uColor3.value as Float32Array;
      c1.set(current.color1);
      c2.set(current.color2);
      c3.set(current.color3);
